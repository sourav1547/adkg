...
with open(".benchmarks/Linux-CPython-3.7-64bit/0002_pcl_detailed.json", "r") as file:
...
nvals_verifybatch = [str(int(t)*3 + 1) for t in tvals_verifybatch]
...
with open(".benchmarks/Linux-CPython-3.7-64bit/0001_hbavss_dummy_pcl.json", "r") as file:
    logdata = file.read().replace("\n", "")
logbenchmarks = json.loads(logdata)["benchmarks"]
for entry in logbenchmarks:
...
try:
    with open(".benchmarks/amt_benchmarks/vssresults.csv", "r") as file:
    ...
    amt = False
    amt_plotting_ver_arr = [0] * 100
    plotting_deal_arr = [0] * 100
...
plt.title("PolyCommitHB vs AMT PC Verification Performance",
          fontsize=title_size)
plt.xticks(n_pos[:len(nvals_verifybatch[::2])], nvals_verifybatch[::2])
plt.legend(loc="best")
...
def draw_fixed_multiple_e2e(fixed_multuple, scenario_name, file_name,
    ...
    plt.plot(td_per_party_per_proof_mean_c0, linestyle='-', marker='o',
             color=color1, label="HbACSS0+PolycommitHB")
    if amt:
        plt.plot(td_per_party_per_proof_mean_c1, linestyle='-', marker='o',
...
with open(".benchmarks/Linux-CPython-3.7-64bit/0003_hbacss2_dummy_pcl.json", "r") as file:
    logdata = file.read().replace("\n", "")
logbenchmarks = json.loads(logdata)["benchmarks"]
for entry in logbenchmarks:
    t = entry["params"]["t"]
...
for i, elem in enumerate(td_points_c0):
    ...
    t = elem[1]
    n = 3 * t + 1
    index = nvals_verifybatch.index(str(3 * t + 1))
